/**
 * Register -> MatchBook records. Pure: takes the parsed register, returns
 * documents. No file access, no database, so the same mapping backs both the
 * seed file and the live upsert and the two can never drift apart.
 */
...
export const transform = (reg, { horizonWeeks = 6, maxPerListing = 6 } = {}) => {
   ...
   */
  const HORIZON_WEEKS = horizonWeeks;
  const MAX_PER_LISTING = maxPerListing;
  ...
  for (const e of [...reg.events, ...reg.recurring_community_happenings]) {
    ...
    for (const day of occurrences(e)) {
      ...
      events.push(compact({
        sourceId: occurrenceId(e.id, day),
        importedFrom: DATASET,
